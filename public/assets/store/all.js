...
(function() {
  var show_product_image;

  show_product_image = function() {
    var img_str, show_image, str_arr;
    str_arr = [];
    show_image = '';
    $("[id^='option_types'] :selected").each(function(i) {
      var str;
      str = ($(this)).data("image");
      if (str !== '') {
        return str_arr.push(str);
      }
    });
    img_str = str_arr.join('_');
    $("li.tmb-all a").each(function(i) {
      if ($(this).attr('href').match(img_str)) {
        return show_image = $(this).attr('href');
      }
    });
    if (show_image !== '') {
      return $('#main-image img').attr('src', show_image);
    }
  };

  $(function() {
    return $("[id^='option_types']").change(function() {
      var total;
      show_product_image();
      total = 0;
      $("[id^='option_types'] :selected").each(function(i) {
        var price, str;
        str = $(this).text();
        price = parseFloat(str.match(/\d\d\.\d\d/g));
        if (price) {
          return total += price;
        }
      });
      return $('.price.selling').text('$' + total.toFixed(2));
    });
  });
